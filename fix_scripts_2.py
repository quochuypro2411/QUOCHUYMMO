...
files = ['deposit.html', 'checkout.html', 'history.html']
...
for file in files:
    ...
    # We want to replace the sequence of script tags starting with firebase-app-compat up to common.js or sync.js
    new_content = re.sub(r'<script src="https://www\.gstatic\.com/firebasejs[^>]+></script>\s*<script src="https://www\.gstatic\.com/firebasejs[^>]+></script>\s*<script src="https://www\.gstatic\.com/firebasejs[^>]+></script>\s*<script src="js/firebase-config\.js[^>]*></script>\s*<script src="js/common\.js[^>]*></script>(?:\s*<script src="js/sync\.js[^>]*></script>)?', script_block.strip(), content)
    ...
    if new_content != content:
        ...
        print(f'Failed to match pattern in {file}')
